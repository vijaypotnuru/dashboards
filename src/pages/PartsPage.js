...
const PartsPage = ({ dashboard, common, account }) => {
  ...
  console.log("fetchDatasdasdasdassdasdas", fetchedData);
  const reFecthData = async () => {
    console.log("HI Im Recalled");
    setIsFetching(true);
    try {
      const response = await ApiServices.postRequest(getAllPartsRoute, filterValues);
  ...
  const handleSubmit = async (data) => {
    ...
    setIsFetching(true);
    try {
      const response = await ApiServices.postRequest(getAllPartsRoute, data);
      console.log("response963555555", response.data.message);
      setFetchedData(response.data.message);
  ...
  console.log("fetchedData123456789", fetchedData);
