...
const MandalPage = ({ dashboard, showAlert, account }) => {
  ...
  const handleSearch = async () => {
    console.log("searcdsdadadadh", formValues);

    setLoading(true);

    await fecthMandalData();
    setLoading(false);
  };
    ...
    <Page title={pageActions.pagename}>
      <Container maxWidth="xl">
        ...
        <>
          ...
          {!isLoading && <MandalsList pageActions={pageActions} loading={fetchLoading} mandalList={fetchedData.mandal} handleEdit={handleEdit} handleDelete={handleDelete} />}
        </>
      </Container>
    </Page>
  );
