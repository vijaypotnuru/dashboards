...
const CategoryList = ({ loading, showAlert, categoryList, handleEdit, pageActions, handleDelete }) => {
    ...
    { name: "category_name", label: "Category Name" },
    {
      ...
      options: {
        customBodyRender: (value, tableMeta, updateValue) => {
          var index = categoryList.findIndex((e) => e.category_pk == value);
          console.log("index", index);
          return (
            <Stack direction="row">
              <Tooltip title={pageActions.edit_perm != 1 ? "You don't have access to edit" : ""}>
                <span>
                  <IconButton color="primary" onClick={(e) => handleEdit(categoryList[index])} disabled={pageActions.edit_perm != 1}>
                    <EditNoteIcon />
                  </IconButton>
                </span>
              </Tooltip>
              <Tooltip title={pageActions.delete_perm != 1 ? "You don't have access to delete" : ""}>
                <span>
                  <IconButton color="error" onClick={(e) => handleConfirmDelete(categoryList[index])} disabled={pageActions.delete_perm != 1}>
                    <DeleteForeverIcon />
                  </IconButton>
                </span>
              </Tooltip>
  ...
  return (
    <Card elevation={1}>
      ...
      {!loading && <CustomMuiDataTable title="" columns={columns} data={categoryList} options={options} />}
    </Card>
  );
};
