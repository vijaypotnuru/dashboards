...
const GroupsList = ({ loading, showAlert, groupsList, handleEdit, pageActions, handleDelete }) => {
    ...
    },
    {
      ...
      options: {
        customBodyRender: (value, tableMeta, updateValue) => {
          var index = groupsList.findIndex((e) => e.group_pk == value);

          return (
            <Stack direction="row">
              <Tooltip title={pageActions.edit_perm != 1 ? "You don't have access to edit" : ""}>
                <span>
                  <IconButton color="primary" onClick={(e) => handleEdit(groupsList[index])} disabled={pageActions.edit_perm != 1}>
                    <EditNoteIcon />
                  </IconButton>
                </span>
              </Tooltip>
              ...
              <Tooltip title={pageActions.delete_perm != 1 ? "You don't have access to delete" : ""}>
                <span>
                  <IconButton color="error" onClick={(e) => handleConfirmDelete(groupsList[index])} disabled={pageActions.delete_perm != 1}>
                    <DeleteForeverIcon />
                  </IconButton>
                </span>
              </Tooltip>
  ...
  const handleConfirmDelete = (data) => {
    showAlertDialog({
      description: "Are you sure? Do you want to delete this group?",
      agreeCallback: async () => {
        showLoading();
  ...
  return (
    <Card elevation={1}>
      ...
      {!loading && <CustomMuiDataTable title="" columns={columns} data={groupsList} options={options} />}
    </Card>
  );
};
