...
const AccessMangementList = ({ common, other, showAlert, getAllAccessPermissions, updateAccessPermission, clearOtherReducer }) => {
            ...
            <Box alignItems="center">
              ...
              <Stack direction="row" spacing={1}>
                <CustomCheckboxWithLabel
                  label="Page Access"
                  checked={accessData[`page_access`] == 1 ? true : false}
                  onChange={(e) => {
                    updateAccessPermission(`page_access`, e.target.checked, data);
                  }}
                />
                <CustomCheckboxWithLabel
                  label="Create"
                  checked={accessData[`add_perm`] == 1 ? true : false}
                  onChange={(e) => {
                    if (accessData[`page_access`] == 1) {
                      updateAccessPermission(`add_perm`, e.target.checked, data);
                    }
                  }}
                />
                <CustomCheckboxWithLabel
                  label="Read"
                  checked={accessData[`view_perm`] == 1 ? true : false}
                  // onChange={(e) => {
                  //   if (accessData[`page_access`] == 1) {
                  //     updateAccessPermission(`view_perm`, e.target.checked, data);
                  //   }
                  // }}
                />
                <CustomCheckboxWithLabel
                  label="Update"
                  checked={accessData[`edit_perm`] == 1 ? true : false}
                  onChange={(e) => {
                    if (accessData[`page_access`] == 1) {
                      updateAccessPermission(`edit_perm`, e.target.checked, data);
                    }
                  }}
                />
                <CustomCheckboxWithLabel
                  ...
                  onChange={(e) => {
                    if (accessData[`page_access`] == 1) {
                      updateAccessPermission(`delete_perm`, e.target.checked, data);
                    }
                  }}
                />
                <CustomCheckboxWithLabel
                  ...
                  onChange={(e) => {
                    if (accessData[`page_access`] == 1) {
                      updateAccessPermission(`approved_perm`, e.target.checked, data);
                    }
                  }}
                />
                {(pageId == 114 || pageId == 176) && (
                  <CustomCheckboxWithLabel
                    label="Survey Access"
                    checked={accessData[`survey_perm`] == 1 ? true : false}
                    onChange={(e) => {
                      if (accessData[`page_access`] == 1) {
                        updateAccessPermission(`survey_perm`, e.target.checked, data);
                      }
                    }}
                  />
                )}
  ...
  console.log("other123456789", other);
