...
export const updateAccessPermission = (name, value, jsonData) => async (dispatch) => {
  ...
  var updatedData = { [`${name}`]: value ? 1 : 0 };
  if (name == "page_access") {
    data = { ...data, [`view_perm`]: value ? 1 : 0 };
    updatedData = { ...updatedData, [`view_perm`]: value ? 1 : 0 };

    if (value == false) {
      data = { ...data, [`add_perm`]: 0, [`edit_perm`]: 0, [`delete_perm`]: 0, [`approved_perm`]: 0 };
      updatedData = { ...updatedData, [`add_perm`]: 0, [`edit_perm`]: 0, [`delete_perm`]: 0, [`approved_perm`]: 0 };
    }
  }
